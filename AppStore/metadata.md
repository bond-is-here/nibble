...
The provider’s [privacy page](https://world.openfoodfacts.org/privacy) was not successfully verified in this audit, so current production retention practices remain unknown. The provider’s [API documentation](https://openfoodfacts.github.io/openfoodfacts-server/api/) currently describes v2 as deprecated but still supported, asks production clients to use a custom `AppName/Version (ContactEmail)` User-Agent, and asks developers to identify their usage through its app/usage form. Resolve the actual owner contact, registration/form expectations, and v2-to-v3 decision before release; do not invent a contact or claim provider approval. Ensure the final privacy manifest, public policy, in-app disclosure, and App Store Connect answers agree with the shipped build. Also resolve the physical-device backup/protection item in the [release checklist](release-checklist.md).
