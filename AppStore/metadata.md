...
The provider’s [privacy page](https://world.openfoodfacts.org/privacy) was not successfully verified in this audit, so current production retention practices remain unknown. The provider’s [API documentation](https://openfoodfacts.github.io/openfoodfacts-server/api/) identifies v3 as the current product API, asks production clients to use a custom `AppName/Version (ContactEmail)` User-Agent, and asks developers to identify their usage through its app/usage form. Nibble now requests product data through v3; resolve the actual owner contact and registration/form expectations before release; do not invent a contact or claim provider approval. Ensure the final privacy manifest, public policy, in-app disclosure, and App Store Connect answers agree with the shipped build. Also resolve the physical-device backup/protection item in the [release checklist](release-checklist.md).
...
The diary and optional body profile are saved locally in a dedicated folder marked excluded from device backups, with complete file protection used for iOS writes. Barcode requests go to Open Food Facts with the barcode and standard network/request information; the saved diary and body profile are not sent. The app has no HealthKit integration or cloud sync. Delete removes an individual diary entry; saved foods and earlier migration records remain. All data in the current app container can be removed with iOS Settings > General > iPhone Storage > Nibble > Delete App. Removing the app or losing the device can lose the diary. Earlier external backup copies remain outside the app’s control. These are implementation disclosures; physical-device lock/backup behavior is still a release gate.
```
